...
public class FridgeService {
    ...
    public ResponseEntity<?> updateFridge(FridgeDto.fridgeSequence fridge, FridgeDto.fridgeCreate fridgeDto) {
        if (!fridgeRepository.existsByFridgeId(fridge.getFridgeSeq())) { return response.fail("냉장고를 찾을 수가 없습니다.", HttpStatus.BAD_REQUEST); }
        Fridge currentFridge = fridgeRepository.findByFridgeId(fridge.getFridgeSeq()).orElseThrow();
    ...
    public ResponseEntity<?> defaultFridge(FridgeDto.fridgeSequence fridge) {
        if (!fridgeRepository.existsByFridgeId(fridge.getFridgeSeq())) { return response.fail("냉장고를 찾을 수가 없습니다.", HttpStatus.BAD_REQUEST); }
        Fridge currentFridge = fridgeRepository.findByFridgeId(fridge.getFridgeSeq()).orElseThrow();
    ...
    public ResponseEntity<?> deleteFridge(FridgeDto.fridgeSequence fridge) {
        if (!fridgeRepository.existsByFridgeId(fridge.getFridgeSeq())) { return response.fail("냉장고를 찾을 수가 없습니다.", HttpStatus.BAD_REQUEST); }
        fridgeRepository.deleteByFridgeId(fridge.getFridgeSeq());
